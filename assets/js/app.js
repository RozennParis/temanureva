...
var app1 = new Vue({
    el: '#exploration-autocomplete', // on vise l'element (div) pour initialiser l'instance VueJS dessus
    delimiters: ['${', '}'], // pour dire à vueJS : tu n'utilises pas ces délimiteurs utilisés par Twig >>> no conflict between VueJs and Twig
    data: {
        autocomplete: '',
        items: []
    ...
    mounted () {
        $('input.autocomplete').autocomplete({
            onAutocomplete: (v) => { // function (v) {return v} >>> on clique, ça fait quelque chose
                ...
                $('#explo_search_bird').val(item.id) // il récupère l'id pour le mettre dans  #explo search birds (champ caché)
            }
        });
    },
    methods: {
